...
export class FormulaireComponent {
  ...
  constructor(private retour: Router) {
    this.registersArray = new ArrayModel();
  }

  passwordPtn = '^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,16}$';
  registersArray: any = { password: '', passwordConfirme: '' };
  validate() {
  ...
  isPasswordValid(): boolean {
    return (
      this.registersArray.password === this.registersArray.passwordConfirme
    );
  }
